...
EOF
  ...
  then
    ...
    test -n "$CABAL_HADDOCK_TARGETS" || CABAL_HADDOCK_TARGETS=all
...
build_compile () {
  ...
  then
    ...
    echo "Skipping dependency install steps (BUILD_PACKAGE_ONLY=y)"
  else
    build_pre_dep
  fi
  ...
  then
    ...
    echo "Skipping post dependency steps (BUILD_DEPS_ONLY=y)"
  else
    build_post_dep
    run_docspec
  fi
